...
client.on('ready', async () => {
  client.user.setActivity(`${BotConf.CustomStatus}`, { type: "STREAMING", url: "https://www.twitch.tv/sesnaker"})
...
client.on('disconnect', () => console.log('ERROR - Bot is disconnecting...'))
client.on('reconnecting', () => console.log('ERROR - Bot reconnecting...'))
client.on('error', err => console.log(`ERROR - ${err}`))
client.on('warn', err => console.log(`ERROR - ${err}`));

process
process.on('unhandledRejection', err => console.log('ERROR - ', err))
process.on('uncaughtException', err => { console.log('ERROR - ', err); process.exit(1); });
